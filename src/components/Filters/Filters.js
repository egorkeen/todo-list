...
function Filters (props) {
  ...
  return (
    <div className="filters">
      ...
      <button onClick={selectActiveTodos} className={`filters__button ${currentFilter === 'all' ? 'filters__button_acitve' : ''}`}>Активные</button>
      <button onClick={selectCompletedTodos} className={`filters__button ${currentFilter === 'all' ? 'filters__button_acitve' : ''}`}>Завершенные</button>
  </div>
  );
};
